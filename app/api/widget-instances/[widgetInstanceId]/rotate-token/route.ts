...
export async function POST(_: NextRequest, { params }: { params: Promise<{ widgetInstanceId: string }> }) {
  ...
  try {
    ...
    await requireChannelPermission({ userId: auth.user.id, channelId, permissionKey: "overlay_publish", overlayId });

    const admin = createServiceClient();
    const token = crypto.randomUUID().replaceAll("-", "") + crypto.randomUUID().replaceAll("-", "");
    const { error } = await admin.from("widget_tokens").upsert({
      widget_instance_id: widgetInstanceId,
      public_token: token,
      revoked: false,
      rotated_at: new Date().toISOString()
    }, { onConflict: "widget_instance_id" });
