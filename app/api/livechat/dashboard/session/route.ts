...
export async function POST() {
  ...
  try {
    ...
    if (!threadId) return NextResponse.json({ error: "discord_thread_create_failed" }, { status: 500 });
    await admin.from("livechat_sessions").update({ discord_thread_id: threadId, updated_at: new Date().toISOString() }).eq("id", session.id);
  } catch (err) {
    const msg = err instanceof Error ? err.message : "discord_thread_create_failed";
